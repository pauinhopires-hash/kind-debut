...
function AdminIndex() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (data) {
        ...
        if (data.perfil !== "admin") navigate({ to: "/" });
      }
      fetchStats();
    };
    init();
  ...
  const fetchStats = async () => {
    const [reqCompra, reqInternas, estoque] = await Promise.all([
      supabase.from("requisicoes").select("id", { count: "exact" }).eq("status", "pendente"),
      supabase.from("requisicoes_internas").select("id", { count: "exact" }).eq("status", "pendente"),
      supabase.from("produtos").select("id", { count: "exact" }).filter("estoque_atual", "lte", "estoque_minimo"),
    ]);
    setStats({
      requisicoesPendentes: reqCompra.count ?? 0,
      requisicoesInternas: reqInternas.count ?? 0,
      produtosBaixoEstoque: estoque.count ?? 0,
    });
  };
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          icon: ClipboardList,
          rota: "/admin/requisicoes-internas",
      ...
      <div className="max-w-2xl mx-auto p-4">
        ...
        {menus.map((group) => (
          <div key={group.group} className="mb-5">
            <h2 className="text-xs uppercase tracking-widest text-gray-500 mb-2 px-1">{group.group}</h2>
