-- Fase 6: aplica um lote de linhas de planilha (já conferidas na prévia do
-- front) numa única transação. Casamento por nome é refeito aqui (não
-- confia no produto_id resolvido na prévia), pra evitar corrida entre a
-- prévia e a confirmação. SECURITY INVOKER (padrão): roda como o usuário
-- que chama, respeitando as RLS já existentes (só admin escreve em
-- produtos/estoque_atual/movimentacoes_estoque).
...
BEGIN
  ...
  LOOP
    ...
    IF v_produto_id IS NULL THEN
      INSERT INTO public.produtos (nome, unidade, setor, local, grupo, subgrupo, valor_unitario, estoque_minimo)
      VALUES (
        v_nome,
...
REVOKE EXECUTE ON FUNCTION public.import_estoque_rows(jsonb, uuid, uuid[], text) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.import_estoque_rows(jsonb, uuid, uuid[], text) TO authenticated;
